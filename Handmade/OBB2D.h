...
- This class encapsulates a 2D oriented bounding box that determines collisions that may occur with
  other oriented bounding boxes and boxes on the same axes, or AABBs. There is a scale variable
  ...
  work out how the OBB's inner up and right axis will be rotated. This rotation object is represented
  as a Transform object, as passing individual Euler angles will cause issues, and in 2D the only 
  actual rotation used is one around the Z axis. It is best to create the transform rotation object
  in the client code before sending it to the OBB class. Even though the bound represents a 2D object, 
  all member variables are floats because that provides more accuracy when calculating the bounds, 
  and will provide better collision precision even if it's just a minute amount.

- There are two IsColliding() functions, one for checking two OBB boxes for intersection, and the 
  other to check for collisions between a OBB and AABB box. Both these routines will use the inner
  private IsColliding() function that does the actual work. This is to prevent duplicate code, 
  because checking OBB-OBB and OBB-AABB use almost the same technique, as AABBs are merely OBBs
  without rotations. The private IsColliding() function is sent an array of two OBBs from the other 
  two IsColliding() functions, and will loop through these two boxes, and project all eight corners
  onto all four axes, using the corners and the up and right axes previously calculated. As soon as
  a separating axis is found, the function's inner main loop will break and return false immediately.
