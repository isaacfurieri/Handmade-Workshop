...
- There are two IsColliding() functions, one for checking two OBB boxes for intersection, and the
  ...
  two IsColliding() functions, and will loop through these two boxes, and project all sixteen corners
  onto all six axes (plus the extra edge axes), using the corners and the up, right and forward axes
  previously calculated. As soon as a separating axis is found, the function's inner main loop will
  break and return false immediately.
